...
validate_lookup_input <- function(text) {
  ...
  if (nrow(parsed) == 0) {
    return(list(
      valid = FALSE,
      data = parsed,
      message = "Enter at least one ARG name before running the lookup."
    ))
  }
  ...
  if (any(!nzchar(parsed$normalized_arg_name))) {
    return(list(
      valid = FALSE,
      data = parsed,
      message = "One or more ARG names could not be normalized. Check the input text."
    ))
  }
  ...
  if (length(missing_cols) > 0) {
    return(list(
      ...
      message = paste0(
        ...
        ". Required columns are SampleID, ARG, and Abundance."
      )
    ))
  }
